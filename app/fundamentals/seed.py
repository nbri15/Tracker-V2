...
from app.fundamentals.early_number_sense_seed import LEVELS, QUESTIONS, STRANDS
...
def seed_fundamentals() -> None:
    """Create or update the Early Number Sense records without duplication."""
    strand_by_code = {}
    for row in STRANDS:
        code = _get(row, 'StrandID', 'code')
        strand = FundamentalStrand.query.filter_by(code=code).first()
        if not strand:
    ...
    for row in LEVELS:
        strand = strand_by_code.get(_get(row, 'StrandID', 'code')) or FundamentalStrand.query.filter_by(code=_get(row, 'StrandID', 'code')).first()
